...
class EmanIndex_PageController extends Omeka_Controller_AbstractActionController
{
    ...
    public function indexcompletAction()
      ...
      foreach($items as $i => $item) {
        ...
        $title = metadata($litem, array('Dublin Core', 'Title'));
      ...
      foreach ($resultats as $i => $valeur) {
        if (isset($valeurs[$valeur['text']])) {
          ...
          $content .= "<span style='font-weight:bold;'>" . $valeur['text'] . '</span> (' . $count . ') <span class="montrer"> + </span><br />';
          ...
          $res = $valeurs[$valeur['text']];
      		...
      		$content .= "<ol class='notices' style='display:none;'>" . $count . " notice$e $v cette  valeur pour le champ <em>" . $fieldName->name . '</em> : ';
          foreach ($res as $itemId => $title) {
            $content .= "<li class='valeur'><a href='" . WEB_ROOT . "/items/show/" . $itemId ."' target='_blank'>" . $title . "</a></li>";
          }
          $content .= "</ol>";
